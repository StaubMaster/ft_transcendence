...
class BarNavigationSections
{
	...
	constructor(bar_id: string, ids: string[][])
	{
		this.bar = document.getElementById(bar_id) as HTMLElement;

		this.selection = [];
		this.sections = [];
		for (let i = 0; i< ids.length; i++)
		{
			this.selection[i] = document.getElementById(ids[i][0]) as HTMLElement;
			this.sections[i] = document.getElementById(ids[i][1]) as HTMLElement;
