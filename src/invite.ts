...
{
	let table = document.getElementById("user-table-online") as HTMLTableElement;
...
function Online_Table_Select(ev: MouseEvent)
{
	...
	(document.getElementById("invite-button-invite") as HTMLButtonElement).disabled = (idx == -1);
}
...
export function Invite_Set(idx: number)
{
	...
	{
		let table = document.getElementById("user-table-online") as HTMLTableElement;
		UsersInvitedID = Number.parseInt(table.rows[idx].cells[0].textContent as string);
		UsersInvitedName = table.rows[idx].cells[1].textContent as string;
		UsersInvitedStatus = table.rows[idx].cells[2].textContent as string;
...
{
	let table = document.getElementById("user-table-invite") as HTMLTableElement;
	userTable.clear(table);
...
function Invite_Table_Select(ev: MouseEvent)
{
	let idx = userTable.MouseEventToRowIndex(ev);
	...
	(document.getElementById("invite-button-accept") as HTMLButtonElement).disabled = (idx == -1);
}
